...
contract('MultiSigWallet', (acounts) => {
    const owners = [acounts[0], acounts[1], acounts[2]];
    const NUM_COMFIRMATIONS_REQUIRED = 2;

    let wallet;
    beforeEach(async () => {
        wallet = await MultiSigWallet.new(owners, NUM_COMFIRMATIONS_REQUIRED);
    });
